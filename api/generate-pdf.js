...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Cash Rheo brand ikonica — dole desno
    try {
      const iconPath = path.join(process.cwd(), 'api', 'branding', 'ikona app cash_rheo_v4_512.png');
      if (fs.existsSync(iconPath)) {
        const iconSize = 22;
        const iconX = W - MR - iconSize;
        const iconY = doc.y + 4;
        doc.image(iconPath, iconX, iconY, { width: iconSize });
        doc.y = iconY + iconSize;
      }
    } catch (e) { /* tiho prelazi ako nešto ne valja sa ikonicom */ }
    const chunks = [];
